...
- **emoji-only chat rows clipped emoji at the top** — `.hs-mc-emoji` had `font-size: calc(1em * 2)` (~26px in chat context) but `line-height: 18px` hardcoded. inline-block emoji reported itself as 18px tall; glyph centered and extended 4px above the line box; `.hs-mc-msg`'s `overflow: hidden` clipped that overflow. emote-bearing rows survived because the emote img forced the line box larger. fix: `line-height: 1` so the inline-block tracks font-size — line box grows to fit the glyph. side effect: emoji-only rows are ~8px taller than text-only rows (matches native twitch chat behavior).
