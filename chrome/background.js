...
browser.alarms?.onAlarm?.addListener((alarm) => {
  if (alarm.name === 'keepalive') {
    // Keep worker alive + ensure EventSub is connected
    if (!eventSubWs && !eventSubReconnectTimer) connectEventSub?.()
  }
});
...
// ═══════════════════════════════════════════════════
// TWITCH EVENTSUB — WHISPER RECEIVER
// Connects to Twitch EventSub WebSocket to receive
// incoming whispers without needing heatsync auth.
// ═══════════════════════════════════════════════════
...
let eventSubSubscribed = false
let twitchUserId = null
const EVENTSUB_URL = 'wss://eventsub.wss.twitch.tv/ws'
const TWITCH_GQL_CLIENT_ID = 'kimne78kx3ncx6brgo4mv6wki5h1ko'

async function getTwitchCookieToken() {
  try {
...
async function resolveTwitchUserId(token) {
  ...
  try {
    ...
    const id = data?.data?.currentUser?.id
    if (id) {
      twitchUserId = id
      log('EventSub: resolved Twitch user ID:', id)
    }
    return id || null
  } catch (e) {
    log('EventSub: failed to resolve user ID:', e.message)
    return null
  }
}

function cleanupEventSub() {
  ...
  eventSubSubscribed = false
}

function scheduleEventSubReconnect() {
  if (eventSubReconnectTimer) return
  const delay = Math.min(1000 * Math.pow(2, eventSubReconnectAttempts), 30000) + Math.random() * 1000
  eventSubReconnectAttempts++
  log(`EventSub: reconnecting in ${Math.round(delay)}ms (attempt ${eventSubReconnectAttempts})`)
...
function resetEventSubKeepalive(timeoutSeconds) {
  ...
  // Add 10s buffer to the server's keepalive timeout
...
async function createEventSubSubscription(token, sessionId, userId) {
  try {
    ...
    if (resp.status === 202 || resp.status === 409) {
      // 202 = created, 409 = already exists (both ok)
      eventSubSubscribed = true
      eventSubReconnectAttempts = 0
      log('EventSub: whisper subscription active')
      return true
    }
    const err = await resp.json().catch(() => null)
    log('EventSub: subscription failed:', resp.status, err?.message || JSON.stringify(err))
    return false
  } catch (e) {
    log('EventSub: subscription error:', e.message)
    return false
  }
}

async function connectEventSub() {
  if (eventSubWs?.readyState === WebSocket.OPEN || eventSubWs?.readyState === WebSocket.CONNECTING) return

  const token = await getTwitchCookieToken()
  if (!token) { log('EventSub: no Twitch token — skipping'); return }

  const userId = await resolveTwitchUserId(token)
  if (!userId) { log('EventSub: no user ID — skipping'); return }

  cleanupEventSub()

  const url = eventSubReconnectUrl || EVENTSUB_URL
  eventSubReconnectUrl = null
  log('EventSub: connecting to', url)
  ...
  try {
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (type === 'session_welcome') {
          ...
          const keepaliveTimeout = msg.payload?.session?.keepalive_timeout_seconds || 10
          resetEventSubKeepalive(keepaliveTimeout)
          log('EventSub: session', eventSubSessionId, 'keepalive:', keepaliveTimeout + 's')

          // Create whisper subscription
          const ok = await createEventSubSubscription(token, eventSubSessionId, userId)
          if (!ok) {
            cleanupEventSub()
            scheduleEventSubReconnect()
          }
          ...
          // Twitch wants us to move to a new URL
          eventSubReconnectUrl = msg.payload?.session?.reconnect_url
          log('EventSub: reconnect requested →', eventSubReconnectUrl)
          cleanupEventSub()
          connectEventSub()
        } else if (type === 'notification') {
          const sub = msg.payload?.subscription
          const evt = msg.payload?.event
          resetEventSubKeepalive(10)

          if (sub?.type === 'user.whisper.message' && evt) {
            log('EventSub: whisper from', evt.from_user_name)
            broadcastToTabs({
              type: 'twitch_whisper',
              data: {
                type: 'whisper',
                user: evt.from_user_name || evt.from_user_login || 'unknown',
                userId: evt.from_user_id || '',
                text: evt.whisper?.text || '',
                color: '#fff',
                time: Date.now(),
                id: evt.whisper_id || ''
              }
          ...
          log('EventSub: subscription revoked:', msg.payload?.subscription?.type)
          eventSubSubscribed = false
        }
      } catch (e) {
        log('EventSub: message parse error:', e.message)
      }
    }
    ...
    log('EventSub: connect error:', e.message)
    cleanupEventSub()
    scheduleEventSubReconnect()
  }
}

// Watch for Twitch login/logout via cookie changes
browser.cookies.onChanged.addListener((changeInfo) => {
  try {
    const c = changeInfo.cookie
    if (c.name !== 'auth-token' || !c.domain.includes('twitch.tv')) return
    if (changeInfo.removed) {
      log('EventSub: Twitch cookie removed — disconnecting')
      twitchUserId = null
      cleanupEventSub()
      if (eventSubReconnectTimer) { clearTimeout(eventSubReconnectTimer); eventSubReconnectTimer = null }
    } else {
      log('EventSub: Twitch cookie set — connecting')
      twitchUserId = null // re-resolve
      eventSubReconnectAttempts = 0
      connectEventSub()
    }
  } catch {}
