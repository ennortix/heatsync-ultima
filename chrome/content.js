...
const hsSched = (() => {
  ...
  for (const ev of ['scroll', 'wheel', 'touchmove', 'pointerdown']) {
    try { window.addEventListener(ev, markBusy, { passive: true, capture: true, signal }) } catch {}
  }
  const _yield = () => {
  ...
  const chunk = async (items, fn, { budgetMs = 4, respectScroll = true } = {}) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try { fn(items[i], i) } catch (e) { try { log(' chunk fn err:', e?.message) } catch {} }
...
function placeAndWireMenu(el, x, y, kbdHandlers = {}) {
  ...
  const onKey = (ev) => {
    ...
    if (ev.key === 'Escape') { ev.preventDefault(); closeEmoteMenu(); return }
    const fn = kbdHandlers[ev.key]
    if (fn) { ev.preventDefault(); try { fn() } catch {} ; closeEmoteMenu() }
  }
...
// Twitch follow via native button click — last-resort cross-follow path. SW
// opens twitch.tv/{slug} in a background tab; this handler waits for Twitch's
// React tree to render the follow button, clicks it (which fires through
// Twitch's own Apollo client so anti-bot passes), handles unfollow confirm
// modal, returns success. Separate listener so the SW ping/click roundtrip
// is async and doesn't block other message paths.
function _onMessageTwitchClick(message, _sender, sendResponse) {
  if (message?.type === 'hs_ping') {
    sendResponse({ ok: true })
    return false
  }
  if (message?.type !== 'hs_click_follow_button') return false
  ;(async () => {
    try {
      const wantFollow = !!message.follow
      // Poll for follow / unfollow button. Twitch renders the button via React
      // after the page hydrates — usually a second or two after tabs.onUpdated
      // 'complete'. Up to 10s budget; channel-not-found / banned channels never
      // surface a button so we fail out gracefully.
      const start = Date.now()
      const FOLLOW_SEL = 'button[data-a-target="follow-button"]'
      const UNFOLLOW_SEL = 'button[data-a-target="unfollow-button"]'
      let followBtn = null
      let unfollowBtn = null
      while (Date.now() - start < 10000) {
        followBtn = document.querySelector(FOLLOW_SEL)
        unfollowBtn = document.querySelector(UNFOLLOW_SEL)
        if (followBtn || unfollowBtn) break
        await new Promise(r => setTimeout(r, 250))
      }
      if (!followBtn && !unfollowBtn) {
        sendResponse({ ok: false, error: 'no_follow_button' })
        return
      }
      // Idempotent: if the current state matches the desired state, success.
      if (wantFollow && unfollowBtn) { sendResponse({ ok: true, idempotent: true }); return }
      if (!wantFollow && followBtn)  { sendResponse({ ok: true, idempotent: true }); return }

      // State change needed — click.
      const btn = wantFollow ? followBtn : unfollowBtn
      btn.click()

      if (!wantFollow) {
        // Unfollow triggers a confirm modal. Wait for it + click confirm.
        const modalStart = Date.now()
        while (Date.now() - modalStart < 3000) {
          const confirmBtn = document.querySelector('button[data-a-target="unfollow-confirm-button"], [role="dialog"] button[data-a-target*="confirm"]')
          if (confirmBtn) { confirmBtn.click(); break }
          await new Promise(r => setTimeout(r, 100))
        }
      }

      // Wait briefly for state to flip (verifies the click registered).
      const flipStart = Date.now()
      while (Date.now() - flipStart < 4000) {
        const fb = document.querySelector(FOLLOW_SEL)
        const ub = document.querySelector(UNFOLLOW_SEL)
        if (wantFollow && ub && !fb) { sendResponse({ ok: true }); return }
        if (!wantFollow && fb && !ub) { sendResponse({ ok: true }); return }
        await new Promise(r => setTimeout(r, 200))
      }
      // Click fired but state didn't flip within timeout — likely succeeded
      // server-side, just UI lagged. Optimistic ok.
      sendResponse({ ok: true, optimistic: true })
    } catch (e) {
      sendResponse({ ok: false, error: e?.message || 'click_failed' })
    }
  })()
  return true
}
if (window.location.hostname.includes('twitch.tv')) {
  chrome.runtime.onMessage.removeListener(_onMessageTwitchClick)
  chrome.runtime.onMessage.addListener(_onMessageTwitchClick)
}
...
function processExistingMessages() {
  ...
  ;(async () => {
    ...
    log(` ⏱️ Processed ${unprocessed.length} messages (${tail.length} tail, ${head.length} head) in ${(performance.now() - startTime).toFixed(0)}ms`)
  })()
}
